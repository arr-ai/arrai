...
func Test_newCharTupleFromTuple(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "0@0",
			...
			ok:   true,
			args: args{t: NewTuple(NewAttr("@", NewNumber(0)), NewAttr(CharAttr, NewNumber(0)))},
		},
		{name: "a@0",
			want: NewStringCharTuple(0, 'a'),
			ok:   true,
			args: args{t: NewTuple(NewAttr("@", NewNumber(0)), NewAttr(CharAttr, NewNumber('a')))},
		},
		{name: "a@42",
			want: NewStringCharTuple(42, 'a'),
			ok:   true,
			args: args{t: NewTuple(NewAttr("@", NewNumber(42)), NewAttr(CharAttr, NewNumber('a')))},
		},
		{name: "no-@",
			want: StringCharTuple{},
			ok:   false,
			args: args{t: NewTuple(NewAttr("at", NewNumber(0)), NewAttr(CharAttr, NewNumber('a')))},
		},
		{name: "no-CharAttr",
			want: StringCharTuple{},
			ok:   false,
