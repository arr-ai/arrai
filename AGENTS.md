...
- Branch protection is a single rule on `master` (not `**/**` — that pattern once covered Dependabot's branches and stopped it rebasing its own PRs). Merges must come through a PR with the nine GitHub Actions jobs green (`Lint`, `Build`, the three `Test (...)` jobs, `Coverage`, `Build Docusaurus site`, `Build and Test Wasm`, `Release`); force-pushes and deletions are blocked, linear history is required, and the rule applies to admins. "Require branch up to date" is off, so merging one PR does not force a rebase of the others.
