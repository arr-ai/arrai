...
This file provides guidance to coding agents (Claude Code, Codex, Cursor, and any other tool that reads `AGENTS.md`) when working with code in this repository. `CLAUDE.md` imports it.
...
- Branch protection is a single rule on `master` (not `**/**` — that pattern once covered Dependabot's branches and stopped it rebasing its own PRs). Nine GitHub Actions jobs are required (`Lint`, `Build`, the three `Test (...)` OS jobs, `Coverage`, `Build Docusaurus site`, `Build and Test Wasm`, `Release`); `Test (slowpath)` joins them once 🎯T34 makes it green on master; force-pushes and deletions are blocked, linear history is required, and the rule applies to admins. "Require branch up to date" is off, so merging one PR does not force a rebase of the others.
- The rule does **not** tick "require a pull request" — the required checks already reject a direct push, so PRs are the only way in. Do not turn it on with a review count of 0: that configuration (reachable only via the API) makes GitHub's merge endpoint return HTTP 500 for any PR whose author is not the merger, while the author's own PRs merge fine (2026-09-15, #753/#754).
- The Netlify checks are **not** required, so Netlify failures are informational only and don't block merge. Don't get stuck waiting for them: use `gh pr checks --required --watch`, or drop `--fail-fast` and filter to the GitHub Actions checks.
