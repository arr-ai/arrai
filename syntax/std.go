	...
	stdScopeOnce.Do(
		func() {
			...
			goStdlib := rel.NewTuple(
				rel.NewNativeFunctionAttr(
					"dict", func(_ context.Context, value rel.Value) (rel.Value, error) {
				...
				rel.NewNativeFunctionAttr(
					"tuple", func(_ context.Context, value rel.Value) (rel.Value, error) {
						switch t := value.(type) {
						case rel.Dict:
							...
							for e := t.DictEnumerator(); e.MoveNext(); {
								...
								if _, ok := keyVal.(rel.String); ok {
									...
									return nil, fmt.Errorf(
										"all keys of arg to //tuple must be strings, not %s",
										rel.ValueTypeAsString(keyVal),
									)
								}
				...
				rel.NewTupleAttr(
					"log",
					rel.NewNativeFunctionAttr(
						"print", func(_ context.Context, value rel.Value) (rel.Value, error) {
							log.Print(rel.Repr(value))
							return value, nil
						},
					),
					createFunc2Attr(
						"printf", func(ctx context.Context, a, b rel.Value) (rel.Value, error) {
				...
				rel.NewNativeFunctionAttr(
					"error", func(_ context.Context, value rel.Value) (rel.Value, error) {
				...
				rel.NewTupleAttr(
					"@internal", rel.NewTupleAttr(
						"xml", createFunc2Attr(
							...
							"encode", func(_ context.Context, value rel.Value) (rel.Value, error) {
...
func createNestedFunc(
	...
	f func(context.Context, ...rel.Value) (rel.Value, error), args fnArgs,
) (rel.Value, error) {
	if nArgs == 0 {
...
func createFunc2(name string, f func(ctx context.Context, a, b rel.Value) (rel.Value, error)) rel.Value {
	return rel.NewNativeFunction(
		name, func(_ context.Context, a rel.Value) (rel.Value, error) {
			return rel.NewNativeFunction(
				name+"$2", func(ctx context.Context, b rel.Value) (rel.Value, error) {
					return f(ctx, a, b)
				},
			), nil
		},
...
func createFunc3(name string, f func(ctx context.Context, a, b, c rel.Value) (rel.Value, error)) rel.Value {
	return rel.NewNativeFunction(
		name, func(_ context.Context, a rel.Value) (rel.Value, error) {
			return rel.NewNativeFunction(
				name+"$2", func(_ context.Context, b rel.Value) (rel.Value, error) {
					return rel.NewNativeFunction(
						name+"$3", func(ctx context.Context, c rel.Value) (rel.Value, error) {
							return f(ctx, a, b, c)
						},
					), nil
				},
...
func newFloatFuncAttr(name string, f func(float64) float64) rel.Attr {
	return rel.NewNativeFunctionAttr(
		name, func(_ context.Context, value rel.Value) (rel.Value, error) {
...
func parseGrammar(_ context.Context, v rel.Value) (rel.Value, error) {
	...
	return rel.NewNativeFunction(
		"parse(<grammar>)", func(_ context.Context, v rel.Value) (rel.Value, error) {
			rule := v.String()
			return rel.NewNativeFunction(
